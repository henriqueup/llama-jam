...
This document outlines the plan for migrating the Lemonfy application (built with Next.js/T3 Stack) to Llama Jam (using React Router).
...
- **Database ORM**: Prisma (initially same backend)
...
1. **Create API Client**
   - Implement a fetch-based API client to replace tRPC
   - Use React Query for data fetching and caching

2. **Define API Endpoints**
   - Create API endpoints for songs, instruments, and other entities
...
### 2. API Layer Transition

**Challenge**: Moving from tRPC to a custom API client.
...
**Total**: 19-30 days (depending on complexity and unforeseen challenges)
